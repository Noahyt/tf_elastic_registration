...
def _interpolate_bilinear(grid,
  ...
  with ops.name_scope(name):
    ...
    total_size = tf.cast(batch_size * height * width, tf.float32)

    # if total_size > tf.constant(np.iinfo(np.int32).max / 8):
...
def dense_image_warp(image, flow, name='dense_image_warp'):
  ...
  with ops.name_scope(name):
    ...
    #     math_ops.range(image_width), math_ops.range(image_height))
